...
public class SurveysEditor extends JDialog {
	...
	public SurveysEditor(Frame owner) {
		...
		JButton addButtonSurvey = new JButton(InternationalisationUtils.getI18nString(SURVEY_ADD));
		...
		scrollNewSurvey.setBounds(139, 339, 300, 63);
		contentPane.add(scrollNewSurvey);

		newSurvey = new JTextArea();
		newSurvey.setLineWrap(true);
		scrollNewSurvey.setViewportView(newSurvey);
		newSurvey.addKeyListener(new KeyAdapter() {
			@Override
		...
		newSurvey.addFocusListener(new FocusListener() {
		...
		helpLabel.setBounds(139, 428, 642, 16);
	...
	private void showInsertSurveyBox() {
		newSurvey.setVisible(true);
		newSurvey.requestFocusInWindow();
	}

	private void insertNewSurvey() {
		if (!newSurvey.getText().trim().equals("")) {
			List<String> values = splitTextArea(newSurvey.getText().trim());
			...
			if (checkForDuplicateValues(values, allSurveyNames)) {
				for (String s : values) {
					ctrl.addNewSurveyWithName(s.trim());
				}
				newSurvey.setText("");
	...
	private void copySelectedSurvey() {
		...
		if (selectedValues.length != 0) {
			if (selectedValues.length == 1) {
				...
				if (name != null) {
					try {
						ctrl.copySurveyWithName((Survey) surveyList.getSelectedValue(), name);
					} catch (Exception e) {
